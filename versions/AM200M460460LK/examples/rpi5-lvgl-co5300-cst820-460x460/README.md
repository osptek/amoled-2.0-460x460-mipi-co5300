...
1. 进入 **`1 System Options`** -> **`S5 Boot`**：
   - 选择 **`Console`**（命令行模式）。
2. 进入 **`1 System Options`** -> **`S6 Auto Login`**：
   - 选择 **`No`**：开机停留在 `login:` 提示符，需手动输入用户名和密码。
   - 选择 **`Yes`**：开机自动以当前默认用户（如 `pi`）登录并直接进入终端控制台。
...
```
/sys/class/drm/card0-DSI-2/status -> connected
/sys/class/drm/card2-HDMI-A-1/status -> disconnected
/sys/class/drm/card2-HDMI-A-2/status -> disconnected
/sys/class/drm/card2-Writeback-1/status -> unknown
/sys/class/drm/card2-Writeback-2/status -> unknown
```

**输出示例**：`/sys/class/drm/card0-DSI-2/status -> connected`

**设备节点**：路径中的 `card0` 对应屏幕设备节点 **/dev/dri/card0**
...
在输出结果中查找你的触摸屏芯片名称，找到对应的 **`Handlers`** 行：

```
I: Bus=0018 Vendor=0000 Product=0000 Version=0000
N: Name="Hynitron CST820 Touchscreen"
P: Phys=input/ts
S: Sysfs=/devices/platform/axi/1000120000.pcie/1f00080000.i2c/i2c-11/11-0015/input/input5
U: Uniq=

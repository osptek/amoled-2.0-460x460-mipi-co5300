 ...
 *
 * Modified for CO5300 DSI Screen (460x460) and Hynitron CST820 Touch (/dev/input/event5)
 *
 ******************************************************************/
#include <unistd.h>
#include <pthread.h>
...
static void configure_simulator(int argc, char ** argv)
{
    ...
    /* 针对 460x460 CO5300 面板的默认分辨率配置 */
    settings.window_width = atoi(env_w ? env_w : "460");
    settings.window_height = atoi(env_h ? env_h : "460");
...
int main(int argc, char ** argv)
{
    ...
    /* 2. 覆盖设置 DRM 显卡设备节点为树莓派 5 的 MIPI-DSI 卡节点 (/dev/dri/card0) */
    setenv("LV_LINUX_DRM_CARD", "/dev/dri/card0", 1);
    ...
    /* 3. 配置 EVDEV 触摸设备节点为 Hynitron CST820 的 /dev/input/event5 */
#if LV_USE_EVDEV
